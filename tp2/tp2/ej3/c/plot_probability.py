...
def plot_probabilities(perceptron, data, exp, filename):
    ...
    colors = ['red', 'blue', 'green', 'yellow', 'purple', 'orange', 'brown', 'pink', 'olive', 'indigo']
    ...
    bars = []
    for j in range(0, 10):
        bars.append(plt.bar(x_axis + width * j, historic[j], width, yerr=stds[j], ecolor='black', capsize=3, color=colors[j]))
...
if __name__ == "__main__":
    ...
    expected_column = ["y"]
    csv = ["noisy_digitsformat-normal.csv", "noisy_digitsformat-normal-fuerte.csv", "noisy_digitsformat-normal-masfuerte.csv", "noisy_digitsformat-uniform.csv"]
    name = ["n", "f", "mf", "u"]
    for i in range(0, len(csv)):
        data, exp = CSVLoader.load(csv[i], False, data_column, expected_column, True)
        plot_probabilities(wrapper.perceptron, data, exp, "probability-" + wrapper.perceptron.optimizer.__name__ + "-" + name[i] + ".png")
