...
from tp1.ladoA.algorithms import a_star, bfs, dfs, greedy, iddfs, a_star_t, greedy_t
...
def test_admissible(create_matrix):
    ...
    for i in range(0, 5):
        ...
        for heuristic in [frontier_color_count, graph_max_distance]:
            root = tree_node_root(matrix)
            start_time = time.time()
            sol, nodes = a_star_t(root, heuristic)
            time_aux.append((time.time() - start_time) * 1000)
            nodes_aux.append(nodes)
        root = tree_node_root(matrix)
        start_time = time.time()
    ...
    print_bar_graph(x, 'Tiempo promedio (en ms)', ['A* (colores)', 'A* (distancia)', 'IDDFS', 'BFS'], 'tiempo_admisible_t.png')
    print_bar_graph(y, 'Nodos expandidos', ['A* (colores)', 'A* (distancia)', 'IDDFS', 'BFS'], 'nodos_admisible_t.png')
    ...
    for i in range(0, 5):
        ...
        time_aux = []
        nodes_aux = []
        for algo in [a_star_t, dfs, greedy_t]:
            root = tree_node_root(matrix)
            start_time = time.time()
            sol, nodes = algo(root, amount_of_nodes)
            time_aux.append((time.time() - start_time) * 1000)
    ...
    print_bar_graph(x, 'Tiempo promedio (en ms)', ['A* (cantidad)', 'DFS', 'Greedy'], 'tiempo_no_admisible_t.png')
    print_bar_graph(y, 'Nodos expandidos', ['A* (cantidad)', 'DFS', 'Greedy'], 'nodos_no_admisible_t.png')
